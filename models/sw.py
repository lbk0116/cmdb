...
class network_management_ip(models.Model):
    ...
    type = fields.Selection([('ilo','ilo'),('hardware','hardware'),('software','software')],string='资产类型')
    device_id = fields.Many2one('cmdb.device',string='设备')
    os_instance_id = fields.Many2one('cmdb.os_instance',string='关联的操作系统实例')

    manage_ip = fields.Char(string='设备管理地址')
    manage_interface = fields.Char(string='设备管理接口')
...
class stp_summary(models.Model):
    ...
    bpdu_guard_default = fields.Selection([('enable','enable'),('disabled','disabled')])
    bpdu_filter_default = fields.Selection([('enable','enable'),('disabled','disabled')])
    bridge_Assurance  = fields.Selection([('enable','enable'),('disabled','disabled')])
    loopguard_default  = fields.Selection([('enable','enable'),('disabled','disabled')])
